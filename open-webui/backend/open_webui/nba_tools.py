...
def get_game_info(
    ...
    """
    Get detailed information about NBA games. You can search for a specific game or 
    multiple games by filtering with season, home team, and away team.

    Args:
        season (Optional[int]): The season year (e.g., 2023 for 2023–24 season). 
            If None, results are not filtered by season.
        home_team (Optional[str]): Name of the home team (e.g., "Warriors" or "Golden State Warriors").
            If None, results are not filtered by home team.
        away_team (Optional[str]): Name of the away team (e.g., "Warriors" or "Golden State Warriors").
            If None, results are not filtered by away team.

    Returns:
        Dict: Game information including period, start_time, time_in_period, etc.
    """
    ...
    try:
        # Input validation
        if season != None:
            if season < 1946 or season > datetime.now().year:
                return {"error": f"Invalid season. Must be between 1946 and {datetime.now().year}"}
        # if not away_team:
        ...
        if USE_MOCK_DATA:
            ...
            for game in MOCK_GAMES:
                if home_team and away_team:
                    ...
                    if (game["home_team"]["name"].lower() == home_team.lower()):
                        matching_games.append(game)
                elif not home_team and away_team:
                    if (game["visitor_team"]["name"].lower() == away_team.lower()):
                        matching_games.append(game)
                else:
                    matching_games.append(game)
        ...
        for team in teams:
            ...
            if home_team != None:
                if home_team.lower() in team.full_name.lower() or home_team.lower() in team.name.lower():
                    home_team_id = team.id
            if away_team != None:
                if away_team.lower() in team.full_name.lower() or away_team.lower() in team.name.lower():
        ...
        for game in all_games:
            if home_team and away_team:
                ...
                if (home_team.lower() in game.home_team.name.lower()):
                    matching_games.append(game)
            elif not home_team and away_team:
                if (away_team.lower() in game.visitor_team.name.lower()):
                    matching_games.append(game)
            else:
                matching_games.append(game)
